...
public class CameraManager : MonoBehaviour {
    ...
    {
        if (moveBobPos == Vector3.zero) return;
        moveBobPos = Vector3.Lerp(moveBobPos, Vector3.zero, moveBobReturnSpeed * Time.deltaTime);
    }

    private void ResetRotation()
    {
        if (moveBobRot == Vector3.zero) return;
        moveBobRot = Vector3.Lerp(moveBobRot, Vector3.zero, moveBobReturnSpeed * Time.deltaTime);
    }
